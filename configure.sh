...
GIT_VERSION=${REZ_BUILD_PROJECT_VERSION}
...
echo -e "[CONFIGURE][ARGS] GIT VERSION: ${GIT_VERSION}"

# We check if the arguments variables we need are correctly set.
# If not, we abort the process.
if [[ -z ${EXTRACT_PATH} || -z ${BUILD_PATH} || -z ${INSTALL_PATH} || -z ${GIT_VERSION} ]]; then
    echo -e "\n"
...
# We run the configuration script of Git.
echo -e "\n"
echo -e "[CONFIGURE] Running the configuration script from Git-${GIT_VERSION}..."
echo -e "\n"
...
echo -e "\n"
echo -e "[CONFIGURE] Finished configuring Git-${GIT_VERSION}!"
echo -e "\n"
